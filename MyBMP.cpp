...
int MyBMP::getMaxLbsLength() {
	int biWidthEx = int((infoheader.biWidth + 3) / 4) * 4;
	int numPixel = infoheader.biHeight * biWidthEx * infoheader.biBitCount / 8;
	int maxLength = numPixel / 8;

	return maxLength;
}

void MyBMP::setImagedata(BYTE * imagedata) {
	//此方法功能为写入位图数据
	
	int numPixel = infoheader.biHeight * infoheader.biWidth * infoheader.biBitCount / 8;
	memcpy(this->imagedata, imagedata, sizeof(BYTE) * numPixel);
}

void MyBMP::saveToFile(const char *filename) {
	...
	if (outputfile){
		...
		if (infoheader.biBitCount < 24)							//判断是否有调色板，如果有的话读取调色板
		{
			...
			// 1<<x = 2^x
			numQuad = 1 << infoheader.biBitCount;
			outputfile.write((char*)(quad), sizeof(RGBQUAD) * numQuad);
		}
		
		int biWidthEx = int((infoheader.biWidth + 3) / 4) * 4;
		int numPixel = infoheader.biHeight * biWidthEx * infoheader.biBitCount / 8;
		outputfile.write((char*)(imagedata), sizeof(BYTE) * numPixel);
		outputfile.close();
	}
}
